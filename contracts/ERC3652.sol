...
contract ERC3652 {
    ...
    function addressOf(address token, uint256 tokenId) public view returns(address) {
        return Create2.computeAddress(0, keccak256(getProxyCode(token, tokenId)));
    }

    function deploy(address token, uint256 tokenId) public returns(address) {
        return Create2.deploy(0, 0, getProxyCode(token, tokenId));
    }

    function getProxyCode(address token, uint256 tokenId) public pure returns(bytes memory code) {
        code = PROXY_CODE;
        assembly ("memory-safe") {
            let ptr := add(code, 0x20)
            let tmp := mload(add(ptr, PROXY_CODE_TOKEN_ADDRESS_BYTE_OFFSET))
            mstore(add(ptr, PROXY_CODE_TOKEN_ADDRESS_BYTE_OFFSET), or(tmp, token))
            mstore(add(ptr, PROXY_CODE_TOKEN_ID_BYTE_OFFSET), tokenId)
        }
    }
}
